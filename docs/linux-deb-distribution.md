...
```text
build/linux/x64/release/plume-pdf_0.0.16+17_amd64.deb
```
...
```bash
dpkg-deb --build --root-owner-group "$pkg_root" /tmp/plume-pdf_0.0.16+17_amd64.deb
```

## 10. 安装测试

```bash
sudo apt install /tmp/plume-pdf_0.0.16+17_amd64.deb
# 如果缺少依赖
sudo apt install -f
```
...
```text
/tmp/plume-pdf_0.0.16+17_amd64.deb
```

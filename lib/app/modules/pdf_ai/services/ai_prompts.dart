...
class AiPrompts {
  ...
  static String documentContextPrompt(PdfAiContext context) {
    ...
    if (context.outline.isEmpty) {
      ...
      for (final PdfOutlineEntry entry in _outlineEntriesForContext(context)) {
        ...
        if (usedCharacters + line.length + 1 > _maxOutlineCharacters) break;
        buffer.writeln(line);
  ...
  static List<PdfOutlineEntry> _outlineEntriesForContext(PdfAiContext context) {
    ...
    if (outline.length <= _maxOutlineEntries) return outline;
  ...
  static String _outlineTitle(String title) {
    ...
    if (normalized.length <= _maxOutlineTitleCharacters) return normalized;
